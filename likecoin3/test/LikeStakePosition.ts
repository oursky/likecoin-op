...
describe("LikeStakePosition", async function () {
    ...
    it("reverts manager ops when paused", async function () {
      ...
      await expect(
        likeStakePosition.write.mintPosition(
          [kin.account.address, mockBookNFT, 1n, 100n, 0n, ""],
  ...
  describe("Manager controls", async function () {
    ...
    it("non-manager cannot mint/update/burn", async function () {
      ...
      await expect(
        likeStakePosition.write.mintPosition(
          [rick.account.address, mockBookNFT, 1n, 100n, 0n, ""],
          { account: rick.account },
        ),
  ...
  describe("Mint / Update / Burn flows", async function () {
    it("manager can mint, update and burn positions", async function () {
      ...
      // Mint
      await likeStakePosition.write.mintPosition(
        [rick.account.address, mockBookNFT, 42n, 123n, 0n, ""],
        { account: deployer.account },
      );
      ...
      expect(pos1.stakedAmount).to.equal(123n);
      expect(pos1.rewardIndex).to.equal(0n);

      // Update
      await likeStakePosition.write.updatePosition([1n, 456n, 7n], {
        account: deployer.account,
      });
      const pos2 = await likeStakePosition.read.getPosition([1n]);
      expect(pos2.stakedAmount).to.equal(456n);
      expect(pos2.rewardIndex).to.equal(7n);
  ...
  describe("Token URI behavior", async function () {
    it("returns baseURI + tokenId when tokenURI not set", async function () {
      const { likeStakePosition, deployer, rick } =
        await loadFixture(deployLSP);
      const mockBookNFT = "0x0000000000000000000000000000000000000001";

      await likeStakePosition.write.setManager([deployer.account.address], {
        account: deployer.account,
      });
      await likeStakePosition.write.setBaseURI(["https://example.com/meta/"], {
        account: deployer.account,
      });

      await likeStakePosition.write.mintPosition(
        [rick.account.address, mockBookNFT, 7n, 1n, 0n, ""],
        { account: deployer.account },
      );

      const uri = await likeStakePosition.read.tokenURI([1n]);
      expect(uri).to.equal("https://example.com/meta/1");
    });

    it("returns explicit tokenURI when provided on mint", async function () {
      const { likeStakePosition, deployer, kin } = await loadFixture(deployLSP);
      const mockBookNFT = "0x0000000000000000000000000000000000000002";

      await likeStakePosition.write.setManager([deployer.account.address], {
        account: deployer.account,
      });
      await likeStakePosition.write.mintPosition(
        [kin.account.address, mockBookNFT, 8n, 1n, 0n, "ipfs://QmTokenUri"],
        { account: deployer.account },
      );

      const uri = await likeStakePosition.read.tokenURI([1n]);
      expect(uri).to.equal("ipfs://QmTokenUri");
    });
  });
});
