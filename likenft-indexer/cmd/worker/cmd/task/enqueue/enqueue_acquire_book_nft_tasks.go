...
var enqueueAcquireBookNFTTasksCmd = &cobra.Command{
	Use: "enqueue-acquire-book-nft-tasks",
	Run: func(cmd *cobra.Command, args []string) {
		ctx := cmd.Context()
		asynqClient := context.AsynqClientFromContext(ctx)

		task, err := task.NewEnqueueAcquireBookNFTTasksTask()
		if err != nil {
			panic(err)
		}
...
func init() {
	EnqueueCmd.AddCommand(enqueueAcquireBookNFTTasksCmd)
}
